...
#pragma omp parallel for
        for (int y = 0; y < H; y++) {
            for (int x = 0; x < W; x++) {
                ...
                curInput[(y*W+x)*3 + 0] = IN.val[2]/255.;
                curInput[(y*W+x)*3 + 1] = IN.val[1]/255.;
                curInput[(y*W+x)*3 + 2] = IN.val[0]/255.;
                cv::Vec3b PROCESS = processedstreamer.at<cv::Vec3b>(y, x);
                curProcessed[(y*W+x)*3 + 0] = PROCESS.val[2]/255.;
                curProcessed[(y*W+x)*3 + 1] = PROCESS.val[1]/255.;
                curProcessed[(y*W+x)*3 + 2] = PROCESS.val[0]/255.;
            }
        }
        ...
        //preview opticalflow
        backwardoflow = cv::Mat::zeros(H,W, CV_32FC3);
#pragma omp parallel for
        for (int y = 0; y < H; y++) {
            for (int x = 0; x < W; x++) {
...
#pragma omp parallel for
        ...
        outputstreamer = cv::Mat::zeros(H,W, CV_8UC3);
#pragma omp parallel for
        for (int y = 0; y < H; y++) {
            for (int x = 0; x < W; x++) {
                float Ox= min(255., max(0., curSolution[(y*W+x)*3+0]*255.));
                float Oy= min(255., max(0., curSolution[(y*W+x)*3+1]*255.));
                float Oz= min(255., max(0., curSolution[(y*W+x)*3+2]*255.));
